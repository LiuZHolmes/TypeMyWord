    ...
    with open(csv_path, encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if 'last_review' in col_idx and row[col_idx['last_review']]:
                try:
                    last_review = datetime.datetime.fromisoformat(
                        row[col_idx['last_review']])
                except Exception:
                    last_review = None
            words.append(Word(id, word, explanation, due,
                         memory_state, scheduled_days, last_review))
    return words
...
def save_words_to_csv(selected_csv: str, words: list[Word]):
    ...
    for row in rows:
        ...
        if word_id in word_map:
            ...
            row_dict["due"] = w.due.isoformat() if hasattr(
                w, "due") and w.due else ""
            # memory_state序列化为json
            if hasattr(w, "memory_state") and w.memory_state:
                s = str(w.memory_state)
                # 用正则提取值
                match = re.search(
                    r"stability: ([\d.]+), difficulty: ([\d.]+)", s)
                if match:
            ...
            row_dict["scheduled_days"] = str(
                w.scheduled_days) if hasattr(w, "scheduled_days") else ""
            row_dict["last_review"] = w.last_review.isoformat() if hasattr(
                w, "last_review") and w.last_review else ""
            # 按field_names顺序输出
